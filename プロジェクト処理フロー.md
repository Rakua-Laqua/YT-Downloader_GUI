...
flowchart TD
    ...
    App_ShowMainWindow["MainWindow.Show()"]
    ...
    App_OnStartup --> App_ShowMainWindow
    App_ShowMainWindow -->|各ViewModelのコンストラクタで設定読込| SettingsRepo_Load
...
flowchart TD
    subgraph ViewModels ["UI制御層 (ViewModels / App)"]
        ...
        SettingsVM_Save["③ SettingsViewModel.SaveSettingsAsync"]
        SettingsVM_Update["SettingsViewModel.UpdateYtDlpAsync<br/>(手動更新コマンド)"]
        DownloadVM_Analyze["⑤ DownloadViewModel.AnalyzeAsync"]
    ...
    subgraph YtDlpService ["yt-dlp 連携層 (Services)"]
        ...
        YtDlp_Ensure["YtDlpClient.EnsureYtDlpUpdatedAsync<br/>(自動更新)"]
    ...
    %% 相互呼び出し関係の接続
    App_OnStartup -->|MainWindow表示後、各VMコンストラクタで設定ロード| SettingsRepo_Load
    
    SettingsVM_Save -->|設定保存| SettingsRepo_Save["SettingsRepository.SaveAsync"]
    SettingsVM_Update -.->|手動アップデート| YtDlp_Update
    ...
    YtDlp_AnalyzeUrl -.->|自動アップデート確認| YtDlp_Ensure
    YtDlp_Download -.->|自動アップデート確認| YtDlp_Ensure
    YtDlp_Ensure -.->|更新処理を内部実行| YtDlp_Update
...
- **処理の流れ**:
  ...
  - 3. **プレイリストの場合**: `--dump-single-json --flat-playlist` オプションを指定し、`RunYtDlpAsync()` を実行してフラットなJSONを取得。`ParsePlaylistFromJson()` を通してマッピング。
  - 4. **単一動画の場合**: `--dump-json --no-playlist` オプションを指定し、`RunYtDlpAsync()` を実行して詳細なJSONを取得。`ParseVideoMetadata()` を通してマッピング。
- **分岐・例外時の挙動**: メソッド全体が `try/catch` で囲まれているため、yt-dlpの実行ファイルが見つからず `GetYtDlpPath()` が `InvalidOperationException` をスローした場合も含め、あらゆる実行時エラー・例外は捕捉される。呼び出し元へは例外を伝播させず、エラーメッセージを付与した失敗状態（`IsSuccess = false`）の `YtDlpAnalyzeResult` を返却する。
