...
flowchart TD
    ...
    A06["⑥ ダウンロード画面: URL解析開始"]
    A07["⑦ yt-dlp解析実行"]
    A08{"⑧ JSON解析結果"}

    A09["⑨ ダウンロード開始指示 / Job生成"]
    A10["⑩ DownloadManagerへ投入"]
    A11["⑪ Job実行制御 / 並列数制限"]
    A12["⑫ yt-dlpダウンロード orchestration"]
    A13["⑬ 引数・形式・ファイル名組立"]
    A14["⑭ 外部プロセス実行 / 進捗解析"]
    A15["⑮ Queue VM更新 / 完了通知"]

    A16["⑯ メタデータ永続化"]
    A17["⑰ キャンセル / リトライ / キュー削除"]

    A18["⑱ ライブラリ読込 / 検索"]
    A19["⑲ ライブラリ選択 / 削除 / 再生 / リンク"]
    ...
    E01(["例外/入力不備: MessageBox表示またはErrorMessage化"])
    E02(["キャンセル: Token通知 / process kill / Canceled状態"])
    E03(["外部ツール失敗: Failed状態 / stderr表示"])
    E04(["配布失敗: throwで停止"])
...
graph LR
    ...
    EXT_WPF(["外部: WPF / XAML / Dispatcher / MessageBox / Dialog"]):::external
    EXT_TOOL(["外部: yt-dlp.exe / ffmpeg.exe / YouTube"]):::external
    EXT_FS(["外部: File System / LocalAppData / Downloads"]):::external
    EXT_PROC(["外部: Process / Explorer / Browser"]):::external
    EXT_WIN32(["外部: user32.dll FlashWindowEx"]):::external
    EXT_JSON(["外部: System.Text.Json"]):::external
    EXT_DI(["外部: Microsoft.Extensions.DependencyInjection"]):::external
    EXT_MVVM(["外部: CommunityToolkit.Mvvm"]):::external
    EXT_DOTNET(["外部: dotnet publish / Compress-Archive / git"]):::external
