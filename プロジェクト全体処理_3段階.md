...
flowchart TD
    ...
    A10 --> A11
    A11 -->|"MaxConcurrentDownloads(1〜8)で待機"| A12
    A11 -->|"キャンセル要求"| E02
    A12 --> A13
    A13 --> A14
    A14 -->|"進捗(stdout/ffmpeg)"| A15
    A14 -->|"403"| A13
    A14 -->|"終了コード!=0"| E03
    A14 -->|"終了コード0: メタデータ保存"| A16
    A16 -->|"完了通知"| A15
    ...
    A19 --> A16
    A19 --> A23
    A19 -->|"ファイル再生(PlayFile)時のみ: ファイル不在で警告"| E01
...
- `NavigateToDownload()`, `NavigateToLibraryAsync()`, `NavigateToSettings()`
  - 役割: 画面切替を開始する `[RelayCommand]`。ただし**現状の `MainWindow.xaml` からはバインドされておらず、UIからは呼ばれていない（実質未使用）**。実際の画面切替の起点は下記 `OnSelectedNavigationChanged` 側。
  - 入力: なし
  - 出力: `void` または `Task`
  - 処理の流れ:
...
- `OnSelectedNavigationChanged(NavigationItem value)`
  - 役割: **画面切替の実際の起点**。`MainWindow.xaml` の各ナビゲーション `RadioButton` は `IsChecked="{Binding SelectedNavigation, Converter=EnumToBoolConverter, ...}"` でバインドされており、選択値が変わるとこのハンドラが呼ばれてナビゲーションを同期する。
...
- `LoadSettings()`
  ...
  - 分岐・例外時の挙動:
    - 音声品質(`DefaultAudioQuality`)のみ `NormalizeSelection` で候補外ならfallbackへ戻す（他の形式/品質は直接代入。全項目の候補照合は⑨ `LoadDefaultsFrom` 側）。
...
- `AutoDetectPaths()`（`[RelayCommand]`。UIから単独実行できる `AutoDetectPathsCommand` でもある）
...
- `DownloadViewModel(IYtDlpClient, IDownloadManager, ISettingsRepository)`
  ...
  - 処理の流れ:
    ...
    - `DownloadManager.GetAllJobs()`（メモリ上の `_allJobs`）から既存Jobを `DownloadQueue` に再表示する。永続化された復元ではなく、同一アプリ実行中に残っているJob（画面を離れて再訪したときなど）を再表示するだけで、アプリ再起動後は対象が無い。
  - 分岐・例外時の挙動:
...
- `AnalyzeAsync()`
  ...
  - 処理の流れ:
    ...
    - 解析中フラグを立て、前回の解析結果（`_currentVideo` / `_currentPlaylist` / `HasAnalyzedResult`）とプレイリスト一覧（`PlaylistItems`）をクリアする。`IsPlaylist` / `ThumbnailUrl` / `VideoTitle` / `ChannelName` / `Duration` / `VideoCount` などの表示用プロパティは開始時点ではクリアせず、解析成功時に新しい値で上書きする（失敗時は前回の表示が残る）。
...
対象ファイル: `YouTubeDownloader/ViewModels/DownloadViewModel.cs`, `YouTubeDownloader/ViewModels/PlaylistItemViewModel.cs`
...
- `StartDownload()`
  ...
  - 処理の流れ:
    ...
    - 選択項目がなければ警告して終了する。
    - プレイリスト名フォルダのパスを決め（この時点では作成せず、実際の作成は⑫ `DownloadAsync` 内）、選択項目ごとに `DownloadJob` と `DownloadJobViewModel` を生成する。
    - 単体動画の場合は1つの `DownloadJob` を生成する。
    - `DownloadQueue` にVMを追加し、`_downloadManager.Enqueue(job)` を呼ぶ。
    - `HasAnalyzedResult` / `InputUrl` / `PlaylistItems` をリセットする。内部の `_currentVideo` / `_currentPlaylist` や表示用タイトル等は残る（次回解析成功時に上書きされる）。
  - 分岐・例外時の挙動:
...
- `PlaylistItemViewModel(VideoMetadata video)`（`PlaylistItemViewModel.cs`）
  - 役割: プレイリスト解析結果の各動画を、選択チェックボックス付きの一覧表示用にラップする。`AnalyzeAsync`(⑥)で生成され `PlaylistItems` に追加、`StartDownload`(⑨)で選択済み項目が `DownloadJob` 化される。
  - 入力: `VideoMetadata video`
  - 出力: `PlaylistItemViewModel` インスタンス
  - 処理の流れ:
    - `Video` を保持する。
    - `Index`(=`PlaylistIndex`)、`Title`、`Channel`、`Duration`、`ThumbnailUrl` は `Video` を参照する読み取り専用プロパティ。
    - チェック状態 `IsSelected` は `[ObservableProperty]` で、**既定値が `true`**（解析直後はプレイリスト全項目が選択済みになる）。
  - 分岐・例外時の挙動:
    - 制御ロジックは持たない単純な表示ラッパー。
...
- `DownloadManager(IYtDlpClient ytDlpClient, IMetadataRepository metadataRepository, ISettingsRepository settingsRepository)`
  - 役割: ダウンロード管理サービスを初期化する。
  - 入力: `IYtDlpClient ytDlpClient`, `IMetadataRepository metadataRepository`, `ISettingsRepository settingsRepository`
  - 出力: `DownloadManager` インスタンス
  - 処理の流れ:
    - yt-dlpクライアント、メタデータRepository、設定Repositoryを保持する。
    - `AppSettings.MaxConcurrentDownloads` を `ClampConcurrency` で1〜8に丸める。
    - 丸めた同時ダウンロード数を初期値にして `SemaphoreSlim` を生成する。
    - `SettingsSaved` を購読し、設定保存後の同時ダウンロード数変更を `ApplyMaxConcurrency` で反映する。
  - 分岐・例外時の挙動:
    - 設定値が1未満なら1、8超過なら8として扱う。
  - 補足: 実行中のジョブは中断せず、以降のジョブ開始から新しい同時ダウンロード数が効く。なお `ConcurrentQueue<DownloadJob> _pendingQueue` は宣言のみで未使用。
...
- `ProcessJobAsync(DownloadJob job)`
  ...
  - 処理の流れ:
    ...
    - `_semaphore.WaitAsync(cts.Token)` で同時実行数を設定値（`MaxConcurrentDownloads`、1〜8）に制限する。
    - セマフォ取得直後に `cts.Token.ThrowIfCancellationRequested()` を呼び、待機中にキャンセルされていればここで `OperationCanceledException` を発生させて即キャンセル扱いにする。
...
- `BuildFilenameTemplate(string template, DownloadJob job)`, `SanitizeFilename(string filename)`, `UpdateLocalFilePath(DownloadJob job, string template)`
  ...
  - 処理の流れ:
    ...
    - 禁止文字の `_` 置換（`SanitizeFilename`）は `{title}` と `{channel}` の置換値に対してのみ適用する。`{id}` は無加工で、組み上げた最終ファイル名全体を再サニタイズはしない（テンプレート自体に禁止文字を含めた場合はそのまま残る）。
    - `{index}` と `{index:02d}` はどちらも `PlaylistIndex` の2桁ゼロ埋め（`ToString("D2")`）で、表記差はない。
...
- `OnJobProgressChanged(object? sender, DownloadJobEventArgs e)`, `OnJobStatusChanged(object? sender, DownloadJobEventArgs e)`
  ...
  - 出力: `void`
  - 処理の流れ:
    - 両ハンドラとも `Application.Current.Dispatcher.Invoke` でUIスレッドへ戻り、Job IDが一致する `DownloadJobViewModel` を探して `UpdateFromJob()` を呼ぶ。
    - `OnJobProgressChanged`（進捗イベント）はここまで。キュー集計や完了通知は行わない。
    - `OnJobStatusChanged`（状態イベント）のみ、続けて `UpdateQueueSummary()` でキュー件数を更新し、`MaybeFlashWhenAllCompleted()` で全完了通知判定も実行する。
  - 分岐・例外時の挙動:
    - VMが見つからなければ更新しない。
...
- `AppSettings`
  ...
  - 出力: 設定値オブジェクト
  - 処理の流れ:
    - yt-dlp/ffmpegパス、自動更新、yt-dlp更新チャンネル、メタデータ取得言語、既定保存先、既定動画/音声形式、既定品質/音声品質、AV1優先(`PreferHighEfficiencyCodecs`)、ファイル名テンプレート、`MaxConcurrentDownloads` を保持する。
    - `DefaultSaveFolder` は既定でユーザーのVideos配下 `YouTubeDownloader`。
  - 分岐・例外時の挙動:
    - Repository読込失敗時はこのデフォルト値が使われる。
  - 補足: `MaxConcurrentDownloads`(既定2) は `DownloadManager` 側で1〜8に丸められ、実際の同時ダウンロード数として使われる。
